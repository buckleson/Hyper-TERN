# Hyper-Tern Development Guidelines

Last updated: 2026-04-12

## What Hyper-Tern Is

Hyper-Tern is a smart model router for **AI agents**. It sits between an agent and its LLM providers, scores each request, and routes it to the cheapest model that can handle it. The dashboard tracks costs, tokens, and messages across any agent that speaks OpenAI-compatible HTTP.

**Supported agents** (configured in `packages/shared/src/agent-type.ts`): OpenClaw, Hermes, OpenAI SDK, Vercel AI SDK, LangChain, cURL, and a generic `other` slot. OpenClaw remains the deepest integration, but no new code or copy should frame Hyper-Tern as OpenClaw-only. When adding examples, prefer "AI agent" as the noun and pick OpenClaw as the worked example rather than the sole target. Hyper-Tern is consumed as a generic OpenAI-compatible HTTP endpoint — there are no first-party OpenClaw plugins in this repo anymore.

Wingman — the gateway tester for sending requests against a Hyper-Tern backend while impersonating any of the supported agents (useful for routing/header-classifier reproductions) — lives in its own repo at [`TheHyper-TERN/wingman`](https://github.com/TheHyper-TERN/wingman) and is hosted at [`wingman.hyper-tern.build`](https://wingman.hyper-tern.build). The dashboard embeds it as an iframe drawer **in dev mode only** — it is dead-code-eliminated from production / self-hosted bundles via `__DEV_MODE__`, and the backend never enables CORS in production. The dev-mode allow-list + CSP `frame-src` is wired in `packages/backend/src/cors-csp-config.ts`.

**Whenever working in dev mode (`/serve`, `npm run dev`, etc.), the Wingman drawer is expected to be available** — open the FAB at the bottom-right of the dashboard (or hit ⌘/Ctrl+Shift+W) and confirm the iframe loads `https://wingman.hyper-tern.build` cleanly. The drawer is part of the dev surface area, so a broken iframe means the dev environment is broken. `/serve` is **dev-only** — never use it to validate production behavior.

## IMPORTANT: Cloud Mode Always

When starting the app for development or testing (e.g. `/serve`), **always use `HYPER_TERN_MODE=cloud`** (the default). Every dev session must use a **fresh PostgreSQL database** via Docker — multiple concurrent dev instances sharing one DB cause cross-run data pollution and intermittent test failures:
...
DB_NAME="hyper_tern_$(openssl rand -hex 4)"
...
To test routing from an OpenClaw agent against a local Hyper-Tern dev server, point OpenClaw at the dev server's OpenAI-compatible proxy directly — there is no plugin anymore:
...
openclaw config set models.providers.hyper-tern '{"baseUrl":"http://localhost:38238/v1","api":"openai-completions","apiKey":"htrn_YOUR_KEY","models":[{"id":"auto","name":"Hyper-Tern Auto"}]}'
openclaw config set agents.defaults.model.primary hyper-tern/auto

# 3. Restart the gateway
openclaw gateway restart
```

The `AgentKeyAuthGuard` accepts any non-`htrn_*` token from loopback IPs in the self-hosted version, so loopback-only testing works even without a valid key. After restarting the backend, also restart the OpenClaw gateway — it doesn't reconnect automatically.
...
│   │   │   ├── agent-api-key.entity.ts      # OTLP ingest keys (htrn_*)
...
│   │   │   ├── services/                    # ingest-event-bus, hyper-tern-runtime, tenant-cache
│   │   │   ├── utils/range.util.ts
...
- **Admin user**: `admin@hyper-tern.build` / `hyper-tern` (email verification email is skipped if Mailgun is not configured — user is created but unverified)
- **Tenant**: `seed-tenant-001` linked to the admin user
- **Agent**: `demo-agent` with OTLP key `dev-otlp-key-001`
- **API key**: `dev-api-key-hyper-tern-001`
...
**Database naming convention:** Always create uniquely-named databases to avoid overlapping other dev/test instances. Use the pattern `hyper_tern_<context>_<random>` (e.g., `hyper_tern_sse_49821`, `hyper_tern_dev_83712`). Create databases via Docker:

```bash
docker exec postgres_db psql -U myuser -d postgres -c "CREATE DATABASE hyper_tern_<name>;"
```

Then set `DATABASE_URL=postgresql://myuser:mypassword@localhost:5432/hyper_tern_<name>` in `.env`.
...
```
User (Better Auth) ──→ Tenant ──→ Agent ──→ AgentApiKey (htrn_*)
                                    │
...
- **AgentApiKey** (`agent_api_keys` table): One-to-one with agent. `htrn_*` format key for OTLP ingestion.
...
| POST | `/api/v1/routing/resolve` | Bearer (htrn_*) | Model resolution |
| POST | `/v1/chat/completions` | Bearer (htrn_*) | LLM proxy (OpenAI-compatible) |
| POST | `/v1/responses` | Bearer (htrn_*) | LLM proxy (OpenAI Responses API) |
| POST | `/v1/messages` | Bearer (htrn_*) | LLM proxy (Anthropic Messages API) |
...
- `MAILGUN_DOMAIN` — Mailgun sending domain (e.g. `mg.hyper-tern.build`).
- `NOTIFICATION_FROM_EMAIL` — Sender email. Default: `noreply@hyper-tern.build`
...
- `HYPER_TERN_MODE` — `selfhosted` or `cloud` (default: `cloud`; auto-`selfhosted` inside Docker via `/.dockerenv`). Self-hosted mode enables loopback auth shortcuts and allows custom-provider URLs with `http://` / private IPs. `local` is accepted as a legacy alias for `selfhosted`.
...
`https://telemetry.hyper-tern.build/v1/report`). The module lives at
...
- `schema_version`, `install_id` (random UUIDv4, persisted once in
  `install_metadata`), `hyper_tern_version`
...
User-facing spec: https://hyper-tern.build/docs/self-hosted#telemetry
...
**Opt-out**: `HYPER_TERN_TELEMETRY_DISABLED=1`. Also auto-disabled when
...
There are **no publishable npm packages** in this repo. `packages/backend`, `packages/frontend`, `packages/shared`, and `packages/hyper-tern` are all `private: true`. Hyper-Tern ships exclusively as the Docker image at `hypertern/hyper-tern` (built from `docker/Dockerfile`).

### `packages/hyper-tern/` is the canonical version

`packages/hyper-tern/` is a **code-free shell package** that exists only to hold the canonical "Hyper-Tern version". It has no `src/`, no tests, no dependencies — just `package.json`, `README.md`, and (after the first release) a `CHANGELOG.md`. The real backend and frontend live under `packages/backend/` and `packages/frontend/` as before.

`.changeset/config.json` has `"ignore": ["hyper-tern-backend", "hyper-tern-frontend", "hyper-tern-shared"]`, so when a contributor runs `npx changeset`, **only `hyper-tern` is a selectable target**. Bumps to `hyper-tern-backend` / `hyper-tern-frontend` / `hyper-tern-shared` are silently discarded. Always target `hyper-tern` regardless of which files you actually changed.
...
Commit the generated `.changeset/*.md` file alongside your code. On merge to `main`, `release.yml` runs `changesets/action`, which opens (or updates) a `chore: version packages` PR bumping `packages/hyper-tern/package.json` and appending to `packages/hyper-tern/CHANGELOG.md`.
...
1. Merge the pending `chore: version packages` PR. `release.yml` detects the version bump in `packages/hyper-tern/package.json` (by diffing `HEAD~1` against `HEAD`) and calls `docker.yml` as a reusable workflow.
2. The `publish` job reads `packages/hyper-tern/package.json`, resolves the version automatically, and pushes `hypertern/hyper-tern:{version}` + `{major}.{minor}` + `{major}` + `sha-<short>` to Docker Hub. The image is multi-arch (amd64 + arm64) and cosign-signed.
...
**Manual override:** `workflow_dispatch` on `Docker → Run workflow` still works for hotfixes and retags. Leave the `version` input blank to use `packages/hyper-tern/package.json`, or pass a semver string to retag an older commit / publish a hotfix version.
...
| Merge of `chore: version packages` PR | `release.yml` runs again, detects the version bump in `packages/hyper-tern/package.json`, and calls `docker.yml` as a reusable workflow. This pushes a new image tag to Docker Hub automatically. |
| Manual `workflow_dispatch` on `Docker` workflow | Reads `packages/hyper-tern/package.json` (or the `version` input override) and pushes a new image tag to Docker Hub. Used for hotfixes and retags. |
