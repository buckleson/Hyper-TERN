#!/usr/bin/env bash
# Hyper-Tern — self-host quick install
#
...
#
# Usage:
#   bash install.sh                  # install into $HOME/Hyper-Tern
#   bash install.sh --dir /opt/Hyper-Tern # install into a custom directory
...
# Override via HYPER_TERN_INSTALLER_SOURCE when you need the installer to
...
log "Waiting for Hyper-Tern to become healthy (up to 120s — first boot pulls images)"
HEALTH_URL="http://127.0.0.1:${HOST_PORT}/api/v1/health"
for _ in $(seq 1 24); do
  if curl -sSf "$HEALTH_URL" >/dev/null 2>&1; then
    log "Hyper-Tern is up."
    cat <<EOF
...
warn "Hyper-Tern did not become healthy within 120s. Check logs with:"
warn "  (cd $INSTALL_DIR && docker compose logs -f hyper-tern)"
exit 1
