...
describe('applyRequestParamDefaults', () => {
  ...
  it('lets configured Hyper-Tern params win over request body values at the same path', () => {
  ...
  it('keeps request body params that are not configured in Hyper-Tern', () => {
    const body: Record<string, unknown> = {
      messages: [],
      temperature: 0.4,
      max_tokens: 2048,
  ...
  it('lets configured nested Hyper-Tern params win while preserving unconfigured nested siblings', () => {
    const body: Record<string, unknown> = {
      messages: [],
