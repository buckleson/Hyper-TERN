 ...
 * link previews instead of the Cloud Hyper-Tern defaults.
 *
 * Only the leading `https://app.hyper-tern.build` is replaced, preserving
 * any path suffix on `og:image` (e.g. `/og-image.png`). When `baseUrl`
 * is empty or matches the default, the input is returned unchanged.
 */
const DEFAULT_BASE = 'https://app.hyper-tern.build';
