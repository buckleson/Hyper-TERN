...
describe('getEncryptionSecret', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env['HYPER_TERN_ENCRYPTION_KEY'];
  ...
  it('returns HYPER_TERN_ENCRYPTION_KEY when it is >= 32 chars', () => {
    const key = 'a'.repeat(32);
    process.env['HYPER_TERN_ENCRYPTION_KEY'] = key;
    expect(getEncryptionSecret()).toBe(key);
  });

  it('returns BETTER_AUTH_SECRET when HYPER_TERN_ENCRYPTION_KEY is not set and secret is >= 32 chars', () => {
    const secret = 'b'.repeat(64);
    process.env['BETTER_AUTH_SECRET'] = secret;
    expect(getEncryptionSecret()).toBe(secret);
  });

  it('prefers HYPER_TERN_ENCRYPTION_KEY over BETTER_AUTH_SECRET', () => {
    const encKey = 'e'.repeat(32);
    const authSecret = 'a'.repeat(64);
    process.env['HYPER_TERN_ENCRYPTION_KEY'] = encKey;
  ...
  it('throws when no key is set', () => {
    expect(() => getEncryptionSecret()).toThrow(
      'Encryption secret required. Set HYPER_TERN_ENCRYPTION_KEY or BETTER_AUTH_SECRET (>=32 chars).',
    );
  });
