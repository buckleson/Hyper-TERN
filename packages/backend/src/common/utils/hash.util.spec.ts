...
describe('verifyKey', () => {
  ...
  it('verifies legacy format (64-char hex without colon)', () => {
    // Legacy format: scrypt with static salt 'hyper_tern-api-key-salt'
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const { scryptSync: scryptLegacy } = require('crypto');
    const legacyHash = scryptLegacy('legacy-key', 'hyper_tern-api-key-salt', 32).toString('hex');
...
describe('keyPrefix', () => {
  it('returns first 12 characters', () => {
    expect(keyPrefix('htrn_abcdefghij')).toBe('htrn_abcdefg');
  });
