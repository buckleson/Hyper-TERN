...
describe('isSelfHosted', () => {
  const originalMode = process.env['HYPER_TERN_MODE'];
  const originalK8s = process.env['KUBERNETES_SERVICE_HOST'];

  afterEach(() => {
    if (originalMode === undefined) delete process.env['HYPER_TERN_MODE'];
    else process.env['HYPER_TERN_MODE'] = originalMode;
  ...
  it('returns true when HYPER_TERN_MODE is "selfhosted"', () => {
    process.env['HYPER_TERN_MODE'] = 'selfhosted';
    expect(isSelfHosted()).toBe(true);
  });

  it('returns true when HYPER_TERN_MODE is legacy "local"', () => {
    process.env['HYPER_TERN_MODE'] = 'local';
    expect(isSelfHosted()).toBe(true);
  });

  it('returns false when HYPER_TERN_MODE is "cloud"', () => {
    process.env['HYPER_TERN_MODE'] = 'cloud';
    delete process.env['KUBERNETES_SERVICE_HOST'];
    expect(isSelfHosted()).toBe(false);
  });

  it('returns false when HYPER_TERN_MODE is "cloud" even inside Docker', () => {
    process.env['HYPER_TERN_MODE'] = 'cloud';
    delete process.env['KUBERNETES_SERVICE_HOST'];
  ...
  it('auto-detects Docker via /.dockerenv when HYPER_TERN_MODE is not set', () => {
    delete process.env['HYPER_TERN_MODE'];
    delete process.env['KUBERNETES_SERVICE_HOST'];
  ...
  it('auto-detects Podman via /run/.containerenv when HYPER_TERN_MODE is not set', () => {
    delete process.env['HYPER_TERN_MODE'];
    delete process.env['KUBERNETES_SERVICE_HOST'];
  ...
  it('returns false when no container marker is present and HYPER_TERN_MODE is not set', () => {
    delete process.env['HYPER_TERN_MODE'];
    delete process.env['KUBERNETES_SERVICE_HOST'];
    mockedExistsSync.mockReturnValue(false);
    expect(isSelfHosted()).toBe(false);
