...
export function getEncryptionSecret(): string {
  const dedicated = process.env['HYPER_TERN_ENCRYPTION_KEY'];
  ...
  // operators have a clear remediation path: set HYPER_TERN_ENCRYPTION_KEY to a
  ...
  if (sessionSecret && sessionSecret.length >= 32) {
    if (!warnedAboutSecretReuse && process.env['NODE_ENV'] === 'production') {
      ...
      logger.warn(
        'HYPER_TERN_ENCRYPTION_KEY not set — falling back to BETTER_AUTH_SECRET for at-rest ' +
          'encryption. Set HYPER_TERN_ENCRYPTION_KEY to a separate 32+ char secret so a session-' +
