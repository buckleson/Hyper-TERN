...
<html>
  <head>
    <meta property="og:url" content="https://app.hyper-tern.build" />
    <meta property="og:image" content="https://app.hyper-tern.build/og-image.png" />
    <meta name="twitter:image" content="https://app.hyper-tern.build/og-image.png" />
  </head>
</html>`;
...
describe('rewriteOgTags', () => {
  ...
  it('returns the input unchanged when baseUrl matches the default', () => {
    expect(rewriteOgTags(SAMPLE_HTML, 'https://app.hyper-tern.build')).toBe(SAMPLE_HTML);
  });
  ...
  it('rewrites all occurrences of the default base to the custom base', () => {
    const result = rewriteOgTags(SAMPLE_HTML, 'https://hyper-tern.example.com');
    expect(result).toContain('content="https://hyper-tern.example.com"');
    expect(result).toContain('content="https://hyper-tern.example.com/og-image.png"');
    expect(result).not.toContain('https://app.hyper-tern.build');
  });
  ...
  it('handles a string with no occurrences gracefully', () => {
    expect(rewriteOgTags('<html></html>', 'https://hyper-tern.example.com')).toBe('<html></html>');
  });
