...
const SAMPLE_HTML =
  '<html><head><meta property="og:url" content="https://app.hyper-tern.build" />' +
  '<meta property="og:image" content="https://app.hyper-tern.build/og-image.png" /></head>' +
  '<body>SPA</body></html>';
...
describe('SpaFallbackFilter', () => {
  ...
  describe('og tag rewriting', () => {
    ...
    it('rewrites og: tags when BETTER_AUTH_URL is provided', () => {
      const filter = loadFilter('https://hyper-tern.example.com');
      const { host, res } = createMockHost('GET', '/');
      filter.catch(exception, host);
      const sent = (res.send as jest.Mock).mock.calls[0][0] as string;
      expect(sent).toContain('content="https://hyper-tern.example.com"');
      expect(sent).toContain('content="https://hyper-tern.example.com/og-image.png"');
      expect(sent).not.toContain('https://app.hyper-tern.build');
    });
    ...
    it('leaves og: tags alone when BETTER_AUTH_URL is empty', () => {
      ...
      filter.catch(exception, host);
      const sent = (res.send as jest.Mock).mock.calls[0][0] as string;
      expect(sent).toContain('content="https://app.hyper-tern.build"');
    });
