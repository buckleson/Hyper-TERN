...
export class NotificationEmailService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.fromEmail =
      ...
      this.configService.get<string>('app.notificationFromEmail', 'noreply@hyper-tern.build');
  }
  ...
  async sendThresholdAlert(
    ...
    if (providerConfig) {
      ...
      const from = providerConfig.domain
        ? `Hyper-Tern <noreply@${providerConfig.domain}>`
        : `Hyper-Tern <${defaultFrom}>`;
    ...
    const from = `Hyper-Tern <${this.fromEmail}>`;
