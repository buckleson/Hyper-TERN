...
describe('buildTelemetryConfig', () => {
  it('is enabled in production when no disable flag is set', () => {
    ...
    expect(cfg.hyperTernVersion).toEqual(expect.any(String));
  });
  ...
  it('is disabled when HYPER_TERN_TELEMETRY_DISABLED is "1" or "true" even in production', () => {
    expect(
      buildTelemetryConfig({ NODE_ENV: 'production', HYPER_TERN_TELEMETRY_DISABLED: '1' }).enabled,
    ).toBe(false);
    expect(
      buildTelemetryConfig({ NODE_ENV: 'production', HYPER_TERN_TELEMETRY_DISABLED: 'true' }).enabled,
    ).toBe(false);
  });

  it('ignores unrecognised disable values (opt-out must be explicit)', () => {
    expect(
      buildTelemetryConfig({ NODE_ENV: 'production', HYPER_TERN_TELEMETRY_DISABLED: 'yes' }).enabled,
    ).toBe(true);
  });
  ...
  it('is disabled when the Hyper-Tern version cannot be read', () => {
    // Simulates a misconfigured image that ships without
    // packages/hyper-tern/package.json. readHyperTernVersion() falls back to
    ...
    jest.isolateModules(() => {
      ...
      expect(cfg.hyperTernVersion).toBe('unknown');
      expect(cfg.enabled).toBe(false);
    });
  });
});

describe('readHyperTernVersion', () => {
  it('returns a semver-shaped string from the real hyper-tern/package.json', () => {
    const version = readHyperTernVersion();
  ...
  it('returns "unknown" when the package.json read fails', () => {
    jest.isolateModules(() => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const mod = require('./telemetry.config') as typeof import('./telemetry.config');
      expect(mod.readHyperTernVersion()).toBe('unknown');
    });
  });
