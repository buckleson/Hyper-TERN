 ...
 *
 * Opt-out: `HYPER_TERN_TELEMETRY_DISABLED=1` silences all outbound traffic.
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.log(
      `Anonymous usage telemetry is enabled. Install ID: ${install.install_id}. ` +
        `To disable: set HYPER_TERN_TELEMETRY_DISABLED=1. What we send: ${TELEMETRY_DOCS_URL}`,
    );
  }
