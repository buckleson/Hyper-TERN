 ...
 * no identifiers beyond the random `install_id` and the Hyper-Tern version.
 * Additive changes keep `schema_version: 1`; breaking changes bump it.
 *
 * Optional fields are flagged as such because installs running older Hyper-Tern
...
export interface TelemetryPayloadV1 {
   ...
   * summed from the same `cost_usd` Hyper-Tern computes at routing time.
