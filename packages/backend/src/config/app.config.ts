...
export const appConfig = registerAs('app', () => ({
  ...
  emailFrom:
    process.env['EMAIL_FROM'] ?? process.env['NOTIFICATION_FROM_EMAIL'] ?? 'noreply@hyper-tern.build',
  // Legacy Mailgun-only (backward compat with older deployments).
  mailgunApiKey: process.env['MAILGUN_API_KEY'] ?? '',
  mailgunDomain: process.env['MAILGUN_DOMAIN'] ?? '',
  notificationFromEmail: process.env['NOTIFICATION_FROM_EMAIL'] ?? 'noreply@hyper-tern.build',
  dbPoolMax: Number(process.env['DB_POOL_MAX'] ?? 20),
  // When true, /api/v1/public/* endpoints expose aggregate stats without auth.
  // Off by default — only Hyper-Tern Cloud's marketing homepage should enable it.
  publicStatsEnabled: process.env['HYPER_TERN_PUBLIC_STATS'] === 'true',
}));
