...
describe('ProxyMessageRecorder', () => {
  ...
  beforeEach(() => {
    ...
    } as unknown as PricingCatalogCacheService;
  ...
  describe('recordSuccessMessage', () => {
    ...
    beforeEach(() => {
      ...
      const repo = { insert: insertMock } as never;
      const pricingCache = { getByModel: getByModelMock } as unknown as PricingCatalogCacheService;
      const eventBus = { emit: emitMock } as unknown as IngestEventBusService;
      recorder.onModuleDestroy();
    ...
    describe('canned Hyper-Tern responses (no_provider / no_provider_key / limit_exceeded / friendly_error)', () => {
      const cases: Array<{ reason: string; errorMessage: string }> = [
        ...
        { reason: 'friendly_error', errorMessage: 'Hyper-Tern internal error' },
      ];

      it.each(cases)(
        'inserts status=error and error_message="$errorMessage" when reason=$reason',
        async ({ reason, errorMessage }) => {
          await recorder.recordSuccessMessage(ctx, 'hyper-tern', 'simple', reason, {
            prompt_tokens: 0,
            completion_tokens: 0,
          });
          expect(insertMock).toHaveBeenCalledTimes(1);
      ...
      it('flips status to error and populates error_message on the dedup-update path', async () => {
        ...
        await recorder.recordSuccessMessage(ctx, 'hyper-tern', 'simple', 'no_provider', {
          prompt_tokens: 0,
          completion_tokens: 0,
        });
