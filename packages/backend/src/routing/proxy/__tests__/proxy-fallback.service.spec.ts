...
import { getProviderParamSpecs, type ProviderParamSpecCatalog } from 'hyper-tern-shared';
...
describe('ProxyFallbackService', () => {
  ...
  let pricingCache: jest.Mocked<PricingCatalogCacheService>;
  ...
  beforeEach(() => {
    ...
    } as unknown as jest.Mocked<PricingCatalogCacheService>;
  ...
  describe('tryForwardToProvider', () => {
    ...
    it('lets saved Hyper-Tern params override inbound body fields at the same path', async () => {
      providerClient.forward.mockResolvedValue({
        response: new Response('{}', { status: 200 }),
        isGoogle: false,
        isAnthropic: false,
