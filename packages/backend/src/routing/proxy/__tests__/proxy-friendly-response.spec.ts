...
describe('proxy-friendly-response', () => {
  describe('getDashboardUrl', () => {
    const prodConfig = {
      get: jest.fn((key: string) => {
        if (key === 'app.betterAuthUrl') return 'https://app.hyper-tern.build';
        return undefined;
      }),
  ...
  describe('buildFriendlyResponse', () => {
    it('returns non-streaming chat completion', async () => {
      ...
      expect(result.meta.model).toBe('hyper-tern');
      expect(result.meta.provider).toBe('hyper-tern');
      ...
      expect((json.id as string).startsWith('chatcmpl-hyper-tern-')).toBe(true);
