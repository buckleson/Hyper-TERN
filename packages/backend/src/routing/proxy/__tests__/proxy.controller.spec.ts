...
describe('ProxyController', () => {
  ...
  it('should expose /v1/models with the Hyper-Tern auto route', () => {
  ...
  it('should return JSON response for non-streaming OpenAI provider', async () => {
    ...
    expect(headers['X-Hyper-Tern-Tier']).toBe('simple');
    expect(headers['X-Hyper-Tern-Model']).toBe('gpt-4o');
    expect(headers['X-Hyper-Tern-Provider']).toBe('OpenAI');
    expect(headers['X-Hyper-Tern-Confidence']).toBe('0.9');
    expect(headers['X-Hyper-Tern-Reason']).toBe('scored');
  });
        ...
        choices: expect.arrayContaining([
          expect.objectContaining({
            message: expect.objectContaining({
              content: expect.stringContaining('[🦚 Hyper-Tern M500]'),
            }),
          }),
        ]),
      }),
  ...
  describe('streaming', () => {
    ...
    it('should pipe streaming responses directly for non-Google', async () => {
      ...
      expect(headers['X-Hyper-Tern-Tier']).toBe('standard');
  ...
  describe('fallback headers', () => {
    it('should set fallback headers when meta has fallbackFromModel', async () => {
      ...
      expect(headers['X-Hyper-Tern-Fallback-From']).toBe('gpt-4o');
      expect(headers['X-Hyper-Tern-Fallback-Index']).toBe('0');
      expect(headers['X-Hyper-Tern-Model']).toBe('claude-sonnet-4');
    });
    ...
    it('should not set fallback headers when meta has no fallbackFromModel', async () => {
      ...
      expect(headers['X-Hyper-Tern-Fallback-From']).toBeUndefined();
      expect(headers['X-Hyper-Tern-Fallback-Index']).toBeUndefined();
    });
    ...
    it('should record failed fallback attempts as separate messages', async () => {
      ...
      expect(headers['X-Hyper-Tern-Fallback-Exhausted']).toBe('true');
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  it('should return primary error status with fallback_exhausted type and X-Hyper-Tern-Fallback-Exhausted header', async () => {
    const mockProviderResp = new Response('primary error', {
      status: 502,
      headers: { 'Content-Type': 'text/plain' },
    });
  ...
  it('should NOT set X-Hyper-Tern-Fallback-Exhausted when error has no failed fallbacks', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(400);
    expect(headers['X-Hyper-Tern-Fallback-Exhausted']).toBeUndefined();
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('should NOT set X-Hyper-Tern-Fallback-Exhausted when a fallback succeeded', async () => {
    const responseBody = { choices: [{ message: { content: 'hello' } }] };
    const mockProviderResp = new Response(JSON.stringify(responseBody), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
