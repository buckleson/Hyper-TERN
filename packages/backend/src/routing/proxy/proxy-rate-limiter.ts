...
export class ProxyRateLimiter implements OnModuleDestroy {
  ...
  checkLimit(userId: string): void {
    ...
    if (entry.count >= RATE_MAX_REQUESTS) {
      throw new HttpException(formatHyperTernError('M201'), HttpStatus.TOO_MANY_REQUESTS);
    }
  ...
  checkIpLimit(ip: string): void {
    ...
    if (entry.count >= IP_RATE_MAX_REQUESTS) {
      throw new HttpException(formatHyperTernError('M202'), HttpStatus.TOO_MANY_REQUESTS);
    }
  ...
  acquireSlot(userId: string): void {
    ...
    if (current >= CONCURRENCY_MAX) {
      throw new HttpException(formatHyperTernError('M203'), HttpStatus.TOO_MANY_REQUESTS);
    }
