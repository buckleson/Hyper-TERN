...
import { formatHyperTernError } from '../../common/errors/error-codes';
...
export class ProxyService {
  ...
  async proxyRequest(opts: ProxyRequestOptions): Promise<ProxyResult> {
    ...
    if (credentials === null) {
      ...
      const content = formatHyperTernError('M100', { provider: route.provider, dashboardUrl });
  ...
  private validatePayload(body: ProxyRequestOptions['body']): void {
    ...
    if (!messages || !Array.isArray(messages) || messages.length === 0) {
      throw new BadRequestException(formatHyperTernError('M300'));
    }
    sanitizeNullContent(messages as Record<string, unknown>[]);
    if (messages.length > MAX_MESSAGES_PER_REQUEST) {
      throw new BadRequestException(formatHyperTernError('M301', { max: MAX_MESSAGES_PER_REQUEST }));
    }
  }
  ...
  private async tryFallbackChain(args: {
    ...
    // The gateway uses the X-Hyper-Tern-Fallback-Exhausted header (set by the
  ...
  private async enforceLimits(tenantId?: string, agentName?: string): Promise<string | null> {
    ...
    return formatHyperTernError('M200', {
  ...
  private buildNoProviderResult(stream: boolean, agentName?: string): ProxyResult {
    const dashboardUrl = getDashboardUrl(this.config, agentName, 'routing');
    const content = formatHyperTernError('M101', { dashboardUrl });
