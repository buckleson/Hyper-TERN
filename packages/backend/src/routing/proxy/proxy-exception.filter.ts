...
import { formatHyperTernError, HyperTernErrorCode } from '../../common/errors/error-codes';
...
const AUTH_ERROR_CODES: Record<string, HyperTernErrorCode> = {
...
export class ProxyExceptionFilter implements ExceptionFilter {
  constructor(private readonly config: ConfigService) {}
    ...
    if (errorCode) {
      const friendly = formatHyperTernError(errorCode);
      ...
      if (isChatClient) {
        ...
        res.status(realStatus).json({
          error: { message: content, type: 'auth_error', code: 'hyper_tern_auth' },
        });
      }
      return;
    }
    ...
    if (isChatClient) {
      const content = status >= 500 ? formatHyperTernError('M500') : message;
      sendFriendlyResponse(res, content, isStream);
      return;
    }
    ...
    const errorMessage =
      status >= 500 ? 'Hyper-Tern encountered an internal error. Try again shortly.' : message;
