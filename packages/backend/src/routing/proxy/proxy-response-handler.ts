...
export function buildMetaHeaders(meta: RoutingMeta): Record<string, string> {
  const headers: Record<string, string> = {
    'X-Hyper-Tern-Tier': meta.tier,
    'X-Hyper-Tern-Model': meta.model,
    'X-Hyper-Tern-Provider': meta.provider,
    'X-Hyper-Tern-Confidence': String(meta.confidence),
    'X-Hyper-Tern-Reason': meta.reason,
    'X-Hyper-Tern-Output-Modality': meta.output_modality ?? 'text',
    'X-Hyper-Tern-Response-Mode': meta.response_mode ?? 'buffered',
  };
  if (meta.specificity_category) {
    headers['X-Hyper-Tern-Specificity'] = meta.specificity_category;
  }
  if (meta.fallbackFromModel) {
    headers['X-Hyper-Tern-Fallback-From'] = meta.fallbackFromModel;
    headers['X-Hyper-Tern-Fallback-Index'] = String(meta.fallbackIndex ?? 0);
  }
  return headers;
}
...
function handleFallbackExhausted(
  ...
  res.status(errorStatus);
  setHeaders(res, metaHeaders);
  res.setHeader('X-Hyper-Tern-Fallback-Exhausted', 'true');
  res.json({
    error: {
      message: sanitizeProviderError(errorStatus, errorBody, process.env.NODE_ENV),
