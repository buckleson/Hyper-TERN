...
export function oauthDoneHtml(success: boolean, nonce?: string, providerLabel = 'Login'): string {
  const message = success ? 'hyper-tern-oauth-success' : 'hyper-tern-oauth-error';
  const text = success
    ? 'Login successful!'
  ...
  return `<!DOCTYPE html>
<html>
<head><title>Hyper-Tern — ${providerLabel}</title></head>
...
<script${nonceAttr}>
try{var bc=new BroadcastChannel('hyper-tern-oauth');bc.postMessage({type:'${message}'});bc.close();}catch(e){}
