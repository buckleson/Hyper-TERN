...
describe('OpenaiOauthService', () => {
  ...
  describe('callback server', () => {
    ...
    it('callback server handles requests on /auth/callback', async () => {
      ...
      expect(res.end).toHaveBeenCalledWith(expect.stringContaining('hyper-tern-oauth-success'));
    });
    ...
    it('callback server returns error HTML when provider sends error param', async () => {
      ...
      expect(res.writeHead).toHaveBeenCalledWith(200, { 'Content-Type': 'text/html' });
      expect(res.end).toHaveBeenCalledWith(expect.stringContaining('hyper-tern-oauth-error'));
      expect(service.getPendingCount()).toBe(0);
    });
