...
import {
  isHyperTernUsableProvider,
  isSupportedSubscriptionProvider,
} from '../../common/utils/subscription-support';
import type { AuthType, ModelRoute } from 'hyper-tern-shared';
import { TIER_LABELS } from 'hyper-tern-shared';
...
export class ProviderService {
  ...
  async removeProvider(
    ...
    const hasOtherUsableAuthType = otherActive.some((record) => isHyperTernUsableProvider(record));
  ...
  private async removeKeyByLabel(
    ...
    const stillHasOtherKeys = matching.some(
      (r) => r.id !== target.id && r.is_active && isHyperTernUsableProvider(r),
    );
  ...
  private async cleanupUnsupportedSubscriptionProviders(agentId: string): Promise<void> {
    ...
    const unsupported = activeProviders.filter(
      (record) => record.auth_type === 'subscription' && !isHyperTernUsableProvider(record),
    );
    ...
    const usableProviders = remainingActive.filter(isHyperTernUsableProvider);
