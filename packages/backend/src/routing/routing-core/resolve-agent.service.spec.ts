...
function makeTenantCache(ensure: jest.Mock): TenantCacheService {
  return { ensure } as unknown as TenantCacheService;
}

describe('ResolveAgentService', () => {
  let findOne: jest.Mock;
  let tenantEnsure: jest.Mock;
  ...
  beforeEach(() => {
    ...
    findOne = jest.fn();
    tenantEnsure = jest.fn();
    svc = new ResolveAgentService(makeAgentRepo(findOne), makeTenantCache(tenantEnsure));
  });
  ...
  it('throws NotFoundException when the agent does not exist for the tenant', async () => {
    tenantEnsure.mockResolvedValue('tenant-1');
  ...
  it('ensures a tenant before resolving the agent', async () => {
    tenantEnsure.mockResolvedValue('tenant-1');
    findOne.mockResolvedValue(null);

    await expect(svc.resolve('user-1', 'demo-agent')).rejects.toBeInstanceOf(NotFoundException);

    expect(tenantEnsure).toHaveBeenCalledWith('user-1');
    expect(findOne).toHaveBeenCalledWith({
      where: { tenant_id: 'tenant-1', name: 'demo-agent', deleted_at: expect.anything() },
    });
  });
  ...
  it('returns and caches the agent on a successful lookup', async () => {
    ...
    expect(tenantEnsure).toHaveBeenCalledTimes(2);
  });
