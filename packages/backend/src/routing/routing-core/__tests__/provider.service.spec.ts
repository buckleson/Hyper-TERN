import type { ModelRoute } from 'hyper-tern-shared';
...
import type { PricingCatalogCacheService } from '../../../pricing-catalog/pricing-catalog-cache.service';
...
describe('ProviderService — route-only cleanup paths', () => {
  ...
  let pricingCache: jest.Mocked<Pick<PricingCatalogCacheService, 'getByModel'>>;
  let routingCache: {
  ...
  beforeEach(() => {
    ...
    svc = new ProviderService(
      ...
      pricingCache as unknown as PricingCatalogCacheService,
...
describe('ProviderService — getFreshSubscriptionCredential', () => {
  const PREV_KEY = process.env.HYPER_TERN_ENCRYPTION_KEY;
  let providerRepo: ReturnType<typeof makeRepo>;
  let svc: ProviderService;

  beforeAll(() => {
    process.env.HYPER_TERN_ENCRYPTION_KEY = 'unit-test-encryption-key-1234567890';
  });
  afterAll(() => {
    if (PREV_KEY === undefined) delete process.env.HYPER_TERN_ENCRYPTION_KEY;
    else process.env.HYPER_TERN_ENCRYPTION_KEY = PREV_KEY;
  });
  ...
  beforeEach(() => {
    ...
    svc = new ProviderService(
      ...
      { getByModel: jest.fn() } as unknown as PricingCatalogCacheService,
      {
