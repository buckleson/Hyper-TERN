...
describe('ResolveAgentService', () => {
  ...
  let mockTenantEnsure: jest.Mock;

  beforeEach(async () => {
    mockFindOne = jest.fn();
    mockTenantEnsure = jest.fn();
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: { ensure: mockTenantEnsure },
        },
      ],
    }).compile();
  ...
  it('ensures the tenant before looking up the agent', async () => {
    mockTenantEnsure.mockResolvedValue('tenant-created');
    mockFindOne.mockResolvedValue(null);

    await expect(service.resolve('user-1', 'my-agent')).rejects.toThrow(NotFoundException);
    await expect(service.resolve('user-1', 'my-agent')).rejects.toThrow('Agent "my-agent" not found');
    expect(mockTenantEnsure).toHaveBeenCalledWith('user-1');
    expect(mockFindOne).toHaveBeenCalledWith({
      where: { tenant_id: 'tenant-created', name: 'my-agent', deleted_at: expect.anything() },
    });
  });

  it('throws NotFoundException when agent not found', async () => {
    mockTenantEnsure.mockResolvedValue('tenant-123');
    mockFindOne.mockResolvedValue(null);
  ...
  it('returns agent when found', async () => {
    ...
    mockTenantEnsure.mockResolvedValueOnce('tenant-123');
