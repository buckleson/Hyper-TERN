...
describe('GeminiOauthController', () => {
  ...
  describe('authorize', () => {
    ...
    it('uses BETTER_AUTH_URL from config when set, ignoring Host header', async () => {
      ...
      configService.get.mockReturnValue('https://hyper-tern.example.com');
  ...
  describe('done', () => {
    ...
    it('returns success HTML when ok=1 with nonce-based CSP', () => {
      ...
      expect(res.send).toHaveBeenCalledWith(expect.stringContaining('hyper-tern-oauth-success'));
    ...
    it('returns error HTML when ok=0 with nonce-based CSP', () => {
      ...
      expect(cspCall).toBeDefined();
      expect(cspCall![1]).toMatch(/^default-src 'none'; script-src 'nonce-[A-Za-z0-9+/=]+'$/);
      expect(res.send).toHaveBeenCalledWith(expect.stringContaining('hyper-tern-oauth-error'));
