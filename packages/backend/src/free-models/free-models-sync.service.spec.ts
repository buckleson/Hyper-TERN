...
describe('FreeModelsSyncService', () => {
  ...
  describe('getAll', () => {
    it('returns bundled providers initially', () => {
      expect(service.getAll().length).toBeGreaterThan(0);
    });
  });

  describe('getLastFetchedAt', () => {
    it('returns the bundled catalog timestamp before any refresh', () => {
      expect(service.getLastFetchedAt()).toBeInstanceOf(Date);
    });

    it('does not update when fetch fails', async () => {
      const before = service.getLastFetchedAt()?.toISOString();
      fetchSpy.mockRejectedValue(new Error('fail'));
      await service.refreshCache();
      expect(service.getLastFetchedAt()?.toISOString()).toBe(before);
    });
