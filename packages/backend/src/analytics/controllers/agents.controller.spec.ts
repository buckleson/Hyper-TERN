...
describe('AgentsController', () => {
  ...
  beforeEach(async () => {
    ...
    mockGetKeyForAgent = jest.fn().mockResolvedValue({ keyPrefix: 'htrn_test1234' });
    mockRotateKey = jest.fn().mockResolvedValue({ apiKey: 'htrn_new_key_123' });
  ...
  it('returns agent key prefix', async () => {
    ...
    expect(result).toMatchObject({ keyPrefix: 'htrn_test1234' });
  ...
  it('returns full apiKey when service returns fullKey', async () => {
    mockGetKeyForAgent.mockResolvedValueOnce({
      keyPrefix: 'htrn_test1234',
      fullKey: 'htrn_full_decrypted',
    });
    const user = { id: 'u1' };
    const result = await controller.getAgentKey(user as never, 'bot-1');

    expect(result).toMatchObject({ keyPrefix: 'htrn_test1234', apiKey: 'htrn_full_decrypted' });
  });
  ...
  it('does not return apiKey when service returns no fullKey', async () => {
    mockGetKeyForAgent.mockResolvedValue({ keyPrefix: 'htrn_test1234' });
    const user = { id: 'u1' };
    const result = await controller.getAgentKey(user as never, 'bot-1');
  ...
  it('rotates agent key and returns new raw key', async () => {
    ...
    expect(result).toEqual({ apiKey: 'htrn_new_key_123' });
  ...
  it('duplicates agent and invalidates cache', async () => {
    ...
    expect(result.apiKey).toBe('htrn_new');
