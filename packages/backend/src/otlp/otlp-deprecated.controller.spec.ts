...
describe('OtlpDeprecatedController', () => {
  ...
  const expectedResponse = {
    error: {
      message:
        ...
        'See https://hyper-tern.build/docs/migration for details.',
      type: 'gone',
      status: 410,
    },
  };
  ...
  describe('traces', () => {
    ...
    it('includes the migration docs URL', () => {
      const result = controller.traces();
      expect(result.error.message).toContain('https://hyper-tern.build/docs/migration');
    });
  });
  ...
  describe('wrong chat completions path', () => {
    ...
    it('includes the correct baseURL hint', () => {
      const result = controller.wrongChatPath();
      expect(result.error.message).toContain('https://app.hyper-tern.build/v1');
    });
  });
});
