...
export class AgentKeyAuthGuard implements CanActivate, OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    // ("hyper_tern-api-key-salt"). The verifyKey path still accepts them for
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    return this.validateHyperTernToken(request, token);
  }
  ...
  private async validateHyperTernToken(request: Request, token: string): Promise<boolean> {
