...
describe('AgentKeyAuthGuard', () => {
  ...
  it('rejects token without htrn_ prefix', async () => {
  ...
  it('rejects htrn_ token shorter than minimum length', async () => {
    const { ctx } = makeContext({ authorization: 'Bearer htrn_ab' });
    await expect(guard.canActivate(ctx)).rejects.toThrow(UnauthorizedException);
    await expect(guard.canActivate(ctx)).rejects.toThrow('Invalid API key format');
    expect(mockCreateQueryBuilder).not.toHaveBeenCalled();
  });

  it('rejects bare htrn_ prefix with no suffix', async () => {
    const { ctx } = makeContext({ authorization: 'Bearer htrn_' });
    await expect(guard.canActivate(ctx)).rejects.toThrow('Invalid API key format');
    expect(mockCreateQueryBuilder).not.toHaveBeenCalled();
  });
  ...
  it('throws UnauthorizedException when API key is not found in DB', async () => {
    ...
    const { ctx } = makeContext({ authorization: 'Bearer htrn_unknown-key' });
  ...
  it('throws UnauthorizedException when API key is expired', async () => {
    const token = 'htrn_expired-key-test';
  ...
  it('uses cached result on second call without querying DB again', async () => {
    const token = 'htrn_cached-key-test';
    mockGetMany.mockResolvedValue([
      {
        id: 'key-4',
        tenant_id: 'tenant-1',
  ...
  describe('dev loopback bypass in development mode', () => {
    ...
    it('allows loopback with non-Hyper-Tern token in dev mode by resolving first active key', async () => {
    ...
    it('rejects non-Hyper-Tern token when no active keys exist in DB', async () => {
      mockFindOne.mockResolvedValue(null);
      const { ctx } = makeContext({ authorization: 'Bearer dev-no-auth' }, '127.0.0.1');
      await expect(guard.canActivate(ctx)).rejects.toThrow('Invalid API key format');
    });

    it('rejects non-Hyper-Tern token from non-loopback IP in dev mode', async () => {
      const { ctx } = makeContext({ authorization: 'Bearer dev-no-auth' }, '192.168.1.100');
      await expect(guard.canActivate(ctx)).rejects.toThrow('Invalid API key format');
    });

    it('rejects loopback with non-Hyper-Tern token in production mode', async () => {
      guard.onModuleDestroy();
    ...
    it('rejects loopback with non-Hyper-Tern token in test mode', async () => {
      guard.onModuleDestroy();
  ...
  it('does not throw when last_used_at update fails', async () => {
    const token = 'htrn_update-fail-key';
    mockGetMany.mockResolvedValue([
      {
        id: 'key-5',
        tenant_id: 'tenant-1',
  ...
  it('evicts the first cache entry when cache reaches MAX_CACHE_SIZE', async () => {
    ...
    const firstFillerHash = testCacheKey('htrn_filler-0');
    for (let i = 0; i < 10_000; i++) {
      internalCache.set(testCacheKey(`htrn_filler-${i}`), {
        tenantId: 't',
        agentId: 'a',
        agentName: 'n',
        userId: 'u',
  ...
  it('evictExpired removes entries whose expiresAt has passed', async () => {
    ...
    const expiredHash = testCacheKey('htrn_expired-cache');
    const validHash = testCacheKey('htrn_valid-cache');
    ...
    const evictToken = 'htrn_trigger-evict-key';
    mockGetMany.mockResolvedValue([
      {
        id: 'key-evict',
  ...
  it('periodic timer fires evictExpired', () => {
    ...
    internalCache.set(testCacheKey('htrn_stale'), {
      tenantId: 't',
      agentId: 'a',
      agentName: 'n',
      userId: 'u',
  ...
  it('onModuleDestroy stops the periodic cleanup timer', () => {
    ...
    internalCache.set(testCacheKey('htrn_leftover'), {
      tenantId: 't',
      agentId: 'a',
      agentName: 'n',
      userId: 'u',
  ...
  it('does not store plaintext tokens in cache', async () => {
    const token = 'htrn_plaintext-check-key';
    mockGetMany.mockResolvedValue([
      {
        id: 'key-pt',
        tenant_id: 'tenant-1',
  ...
  it('caches entries for the configured 5 minute TTL', async () => {
    const token = 'htrn_ttl-check-key';
    mockGetMany.mockResolvedValue([
      {
        id: 'key-ttl',
        tenant_id: 'tenant-1',
  ...
  it('LRU touch on cache hit moves entry to tail of insertion order', async () => {
    const token = 'htrn_lru-touch-key';
    mockGetMany.mockResolvedValue([
      {
        id: 'key-lru',
        tenant_id: 'tenant-1',
    ...
    const internalCache = (guard as any).cache as Map<string, unknown>;
    internalCache.set(testCacheKey('htrn_other-1'), {
      tenantId: 't',
      agentId: 'a',
      agentName: 'n',
      userId: 'u',
