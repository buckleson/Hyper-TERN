...
describe('whitelistResponseHeaders', () => {
  ...
  it('allows x-ratelimit-, x-hyper-tern-, and ratelimit- prefixes (case-insensitive)', () => {
    const h = new Headers();
    ...
    h.append('X-HYPER-TERN-Tier', 'standard');
    h.append('RateLimit-Reset', '60');
    h.append('x-ratelimit-limit', '100');
    h.append('x-hyper-tern-specificity', 'coding');
    ...
    expect(out['x-hyper-tern-tier']).toBe('standard');
    expect(out['ratelimit-reset']).toBe('60');
    expect(out['x-ratelimit-limit']).toBe('100');
    expect(out['x-hyper-tern-specificity']).toBe('coding');
  ...
  it('does not match prefixes mid-string (startsWith only)', () => {
    ...
    h.append('foo-x-hyper-tern-tier', 'cheap');
