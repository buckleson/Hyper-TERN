...
@Injectable()
export class PricingCatalogCacheService implements OnApplicationBootstrap {
  private readonly logger = new Logger(PricingCatalogCacheService.name);
  ...
  getAll(): PricingEntry[] {
    ...
    // here because shared catalog reads are not scoped by user. Returning
    // them would leak one tenant's custom providers
