...
describe('DatabaseSeederService', () => {
  ...
  describe('seedApiKey', () => {
    ...
    it('should store hashed key with null plaintext', async () => {
      ...
      expect(verifyKey('dev-api-key-hyper-tern-001', insertCall.key_hash)).toBe(true);
      expect(insertCall.key_prefix).toBe(keyPrefix('dev-api-key-hyper-tern-001'));
  ...
  describe('seedTenantAndAgent', () => {
    ...
    it('should store agent api key with hashed value and null plaintext', async () => {
      ...
      expect(insertCall.key).toBeNull();
      expect(verifyKey('htrn_dev-otlp-key-001', insertCall.key_hash)).toBe(true);
      expect(insertCall.key_prefix).toBe(keyPrefix('htrn_dev-otlp-key-001'));
    ...
    it('should store the correct OTLP key prefix', async () => {
      ...
      const insertCall = mockAgentKeyRepo.insert.mock.calls[0][0];
      expect(insertCall.key_prefix).toBe('htrn_dev-otl');
    });
