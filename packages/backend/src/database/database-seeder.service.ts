...
const SEED_API_KEY = 'dev-api-key-hyper-tern-001';
const SEED_OTLP_KEY = 'htrn_dev-otlp-key-001';
...
export class DatabaseSeederService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.logger.warn(
      'SECURITY: Default seed credentials are active (admin@hyper-tern.build). Do NOT use in production.',
    );
  }
  ...
  private async seedAdminUser() {
    const existing = await this.checkBetterAuthUser('admin@hyper-tern.build');
    if (existing) return;
