...
export class EncryptApiKeys1771900000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const secret = process.env['HYPER_TERN_ENCRYPTION_KEY'] || process.env['BETTER_AUTH_SECRET'];

    if (!secret || secret.length < 32) {
      log.log(
        'warn',
        'EncryptApiKeys: No encryption secret found. Skipping — set HYPER_TERN_ENCRYPTION_KEY or BETTER_AUTH_SECRET.',
      );
      return;
    }
