...
import { PricingCatalogCacheService } from '../src/pricing-catalog/pricing-catalog-cache.service';
...
beforeAll(async () => {
  ...
  // so PricingCatalogCacheService.inferProvider() resolves the correct provider name)
  ...
  const cache = app.get(PricingCatalogCacheService);
  await cache.reload();
...
describe('Proxy E2E — /v1/chat/completions', () => {
  ...
  it('resolves and attempts to forward to provider', async () => {
    ...
    // so guard-emitted responses never carry X-Hyper-Tern-* headers. If the
    ...
    if (res.status === 401 || res.status === 403) {
      expect(res.headers['x-hyper-tern-tier']).toBeDefined();
      expect(res.headers['x-hyper-tern-model']).toBeDefined();
      expect(res.headers['x-hyper-tern-provider']).toBeDefined();
      ...
      //  - handleProviderError (provider returned 5xx)  → type: 'upstream_error', X-Hyper-Tern-* present
      //  - handleProxyError    (network/throw in proxy) → type: 'server_error',   no X-Hyper-Tern-* headers
  ...
  it('does not return guard 401 envelope when a valid agent key is supplied', async () => {
    ...
    if (res.status === 401) {
      ...
      // type: 'auth_error', code: 'hyper_tern_auth'. Provider 401s come from
      ...
      expect(res.body?.error?.code).not.toBe('hyper_tern_auth');
    }
  });

  it('includes X-Hyper-Tern-* headers when forwarding', async () => {
    const res = await bearer(api().post('/v1/chat/completions'))
      .send({
    ...
    if (res.status !== 500 && res.status !== 400) {
      ...
      expect(res.headers['x-hyper-tern-model']).toBeDefined();
      expect(res.headers['x-hyper-tern-provider']).toBeDefined();
      expect(res.headers['x-hyper-tern-confidence']).toBeDefined();
    }
  });
