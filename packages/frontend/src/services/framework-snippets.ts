...
const STORAGE_KEY = 'hyper_tern_setup_framework';
const TOOLKIT_STORAGE_KEY = 'hyper_tern_setup_toolkit';
const OPENAI_LANG_STORAGE_KEY = 'hyper_tern_setup_openai_lang';
...
const hyperTern = createOpenAI({
  baseURL: "${baseUrl}",
  apiKey: "${apiKey}",${vercelHeaders}
});

const { text } = await generateText({
  model: hyperTern("auto"),
  prompt: "Hello",
});`,
    },
    {
...
export function getOpenClawSnippet(baseUrl: string, apiKey: string): string {
  // Hyper-Tern's cloud proxy speaks OpenAI Chat Completions
  ...
  const providerJson = JSON.stringify({
    ...
    apiKey,
    models: [{ id: 'auto', name: 'Hyper-Tern Auto' }],
  });
  return `openclaw config set models.providers.hyper-tern '${providerJson}'
 ...
 * gymnastics. Pin the default model to Hyper-Tern's `auto` route so Claude
...
export function getOpenClawDisableSnippet(model: string): string {
  return `openclaw config unset models.providers.hyper-tern
openclaw config unset agents.defaults.models.hyper-tern/auto
