...
export function monitorOAuthPopup(
  ...
  const handleMessage = (data: unknown) => {
    ...
    if (data.type === 'hyper-tern-oauth-success') handleResult(true);
    else if (data.type === 'hyper-tern-oauth-error') handleResult(false);
  };

  // BroadcastChannel: works even when COOP severs window.opener
  let pollRef: ReturnType<typeof setInterval>;
  try {
    bc = new BroadcastChannel('hyper-tern-oauth');
