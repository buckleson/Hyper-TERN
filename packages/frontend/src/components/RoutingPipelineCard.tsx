...
export function buildPipelineHelp(
  ...
  if (specificity) {
    steps.push({
      ...
      desc: 'Hyper-Tern semantically analyzes the query, and if it matches an active task-specific tier (coding, image generation\u2026), it is routed to the corresponding model.',
    });
  }
  ...
  steps.push({
    ...
    desc: complexity
      ? 'Hyper-Tern semantically analyzes the query, scores its complexity, and assigns it to a tier ranging from \u201csimple\u201d to \u201creasoning\u201d.'
  ...
  return (
    <div class="routing-pipeline-help-steps">
      <p class="routing-pipeline-help-summary">
        Routing is a powerful technique that allows Hyper-Tern to intercept queries on the fly and
