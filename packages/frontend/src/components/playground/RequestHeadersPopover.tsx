...
export function isBlockedHeaderKey(key: string): boolean {
  ...
  return lower.startsWith('x-hyper-tern-');
}
    ...
    <Show when={props.open}>
      ...
      >
        ...
        <p class="playground-headers__hint">
          Added to every provider call for this playground. Hyper-Tern-managed headers (Authorization,
          Content-Type, x-hyper-tern-*) are ignored.
        </p>
                ...
                >
                  ...
                  <Show when={blocked()}>
                    <p class="playground-headers__warning">
                      {entry.key.trim()} is managed by Hyper-Tern and will be dropped.
                    </p>
                  </Show>
                </div>
              );
