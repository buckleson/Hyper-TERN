...
const AnthropicOAuthDetailView: Component<Props> = (props) => {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      setState(authState);
      const opened = window.open(url, 'hyper-tern-anthropic-oauth', 'noopener,noreferrer');
      if (!opened) {
        toast.error(
      ...
      <Show when={showConnectFlow()}>
        <div class="anthropic-detail__primary">
          <p class="provider-detail__hint">
            Sign in with your Claude Pro or Max account — Hyper-Tern will route through your
