...
const OpenClawSetup: Component<Props> = (props) => {
  ...
  const masked = () => (props.keyPrefix ? `${props.keyPrefix}...` : 'htrn_YOUR_KEY');
  ...
  return (
    <div class="setup-agents-card">
      <p class="setup-step__desc">
        Register Hyper-Tern in your OpenClaw config to route each request to the best provider using
        the model <code class="setup-model-hint__code">hyper-tern/auto</code>
      </p>
      ...
      <Show when={subTab() === 'wizard'}>
        ...
        <div class="setup-onboard-fields" role="list" aria-label="Configuration values">
          ...
          <OnboardField label="Endpoint ID" value="hyper-tern" copyable />
        </div>
      </Show>
    </div>
  );
