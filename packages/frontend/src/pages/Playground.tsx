...
const REQUEST_HEADERS_STORAGE_KEY = 'hyper-tern.playground.requestHeaders';
...
const Playground: Component = () => {
  ...
  const [historyOpen, setHistoryOpen] = createSignal(
    localStorage.getItem('hyper-tern.playground.recentOpen') !== 'false',
  );
  const toggleRecent = () => {
    const next = !historyOpen();
    setHistoryOpen(next);
    localStorage.setItem('hyper-tern.playground.recentOpen', String(next));
  };
  ...
  const handleNewPlayground = () => {
    ...
    sessionStorage.removeItem('hyper-tern.playground.lastRun');
  ...
  createEffect(() => {
    ...
    const runId = searchParams.run || sessionStorage.getItem('hyper-tern.playground.lastRun');
  ...
  const handleSubmit = () => {
    ...
    setViewingHistory(null);
    setSearchParams({ run: runId });
    sessionStorage.setItem('hyper-tern.playground.lastRun', runId);
  };
  ...
  return (
    <div class="playground" style={{ 'padding-bottom': `${promptHeight() + 48}px` }}>
      <Title>Playground · Hyper-Tern</Title>
      <Meta
        name="description"
