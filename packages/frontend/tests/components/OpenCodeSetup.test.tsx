...
describe("OpenCodeSetup", () => {
  ...
  it("renders a paste-ready opencode.json block with a placeholder when the full key is unavailable", async () => {
    ...
    expect(container.textContent).toContain('"apiKey": "htrn_YOUR_KEY"');
    expect(container.textContent).not.toContain('"apiKey": "htrn_live..."');
    ...
    await vi.waitFor(() => {
      expect(writeText).toHaveBeenCalledWith(expect.stringContaining('"apiKey": "htrn_YOUR_KEY"'));
    });
    expect(writeText).not.toHaveBeenCalledWith(expect.stringContaining("htrn_live..."));
  });
  ...
  it("reveals and hides the full API key without changing the copy payload", async () => {
    ...
    expect(container.textContent).toContain('"apiKey": "htrn_live..."');
    expect(container.textContent).not.toContain("htrn_secret");

    fireEvent.click(screen.getByLabelText("Reveal API key"));
    expect(container.textContent).toContain('"apiKey": "htrn_secret"');
    ...
    await vi.waitFor(() => {
      expect(writeText).toHaveBeenCalledWith(expect.stringContaining('"apiKey": "htrn_secret"'));
    });
  });
});
