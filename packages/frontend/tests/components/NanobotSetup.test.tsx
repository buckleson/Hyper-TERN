...
describe("NanobotSetup", () => {
  ...
  it("emits the baseUrl verbatim into the JSON (no /v1 stripping)", () => {
    ...
    // /v1 on Hyper-Tern — preserve the suffix unlike the Anthropic-compatible
    // Claude Code setup.
  ...
  it("masks the API key when only a prefix is available", () => {
    const { container } = render(() => (
      <NanobotSetup {...baseProps} keyPrefix="htrn_abc" />
    ));
  ...
  it("toggles the API key reveal when the eye button is clicked", () => {
    const { container } = render(() => (
      <NanobotSetup {...baseProps} apiKey="htrn_full_secret_value" keyPrefix="htrn_full" />
    ));
