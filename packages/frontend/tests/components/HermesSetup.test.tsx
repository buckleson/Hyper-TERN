...
describe("HermesSetup", () => {
  ...
  it("renders description text", () => {
    ...
    expect(container.textContent).toContain("Point Hermes at the Hyper-Tern endpoint to route requests across multiple models");
  });
  ...
  it("shows key prefix when keyPrefix provided", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} keyPrefix="htrn_abc" />
    ));
  ...
  it("shows eye toggle on CLI tab when apiKey provided", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} apiKey="htrn_full_key" keyPrefix="htrn_ful" />
    ));
  ...
  it("reveals key in CLI when eye toggle clicked", () => {
    ...
    expect(container.textContent).not.toContain("htrn_full_key");
    fireEvent.click(container.querySelector('[aria-label="Reveal API key"]')!);
    expect(container.textContent).toContain("htrn_full_key");
  });
  ...
  it("copies yaml config when copy button in code block is clicked", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} apiKey="htrn_test_key" keyPrefix="htrn_te" />
    ));
  ...
  it("renders EyeIcon with closed state by default", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} apiKey="htrn_test" keyPrefix="htrn_t" />
    ));
  ...
  it("shows masked key in code block when no apiKey", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} keyPrefix="htrn_pre" />
    ));
    expect(container.textContent).toContain("htrn_pre...");
  });
  ...
  it("shows eye toggle on wizard API Key field when apiKey provided", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} apiKey="htrn_wiz" keyPrefix="htrn_wi" />
    ));
    const segment = container.querySelector(".setup-segment--full");
    fireEvent.click(segment!.querySelectorAll(".setup-segment__btn")[1]);
  ...
  it("reveals API key in wizard field when eye toggle clicked", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} apiKey="htrn_wiz_full" keyPrefix="htrn_wi" />
    ));
    const segment = container.querySelector(".setup-segment--full");
    fireEvent.click(segment!.querySelectorAll(".setup-segment__btn")[1]);
    expect(container.textContent).not.toContain("htrn_wiz_full");
    fireEvent.click(container.querySelector('[aria-label="Reveal API key"]')!);
    expect(container.textContent).toContain("htrn_wiz_full");
  });
  ...
  it("copies CLI snippet with full key to clipboard when revealed", async () => {
    ...
    const { container } = render(() => (
      <HermesSetup {...defaultProps} apiKey="htrn_clip_key" keyPrefix="htrn_cli" />
    ));
    fireEvent.click(container.querySelector('[aria-label="Reveal API key"]')!);
    const cliBlock = container.querySelector(".setup-cli-block");
    const copyBtn = cliBlock!.querySelector('[aria-label="Copy to clipboard"]');
    ...
    await vi.waitFor(() => {
      ...
      expect(copiedText).toContain("htrn_clip_key");
  ...
  it("wizard tab uses custom base URL", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} baseUrl="https://app.hyper-tern.build/v1" />
    ));
    const segment = container.querySelector(".setup-segment--full");
    fireEvent.click(segment!.querySelectorAll(".setup-segment__btn")[1]);
    expect(container.textContent).toContain("https://app.hyper-tern.build/v1");
  });

  it("wizard and CLI tabs have independent key reveal state", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} apiKey="htrn_independent" keyPrefix="htrn_in" />
    ));
    // Reveal key on CLI tab
    fireEvent.click(container.querySelector('[aria-label="Reveal API key"]')!);
    expect(container.textContent).toContain("htrn_independent");
    ...
    expect(apiKeyField?.textContent).toContain("htrn_in...");
    expect(apiKeyField?.textContent).not.toContain("htrn_independent");
  });

  it("CLI tab api_key field has eye toggle and copy button", () => {
    const { container } = render(() => (
      <HermesSetup {...defaultProps} apiKey="htrn_cli_field" keyPrefix="htrn_cl" />
    ));
    const fields = container.querySelectorAll(".setup-onboard-fields__row");
    const apiKeyField = Array.from(fields).find((f) =>
