...
describe("ClaudeCodeSetup", () => {
  ...
  it("renders the JSON settings block with Hyper-Tern auto model and ANTHROPIC vars", () => {
    const { container } = render(() => <ClaudeCodeSetup {...baseProps} />);
  ...
  it("masks the API key when only a prefix is available", () => {
    const { container } = render(() => (
      <ClaudeCodeSetup {...baseProps} keyPrefix="htrn_abc" />
    ));
  ...
  it("uses 'htrn_YOUR_KEY' placeholder when neither full key nor prefix is set", () => {
    const { container } = render(() => <ClaudeCodeSetup {...baseProps} />);
  ...
  it("toggles the API key reveal when the eye button is clicked", () => {
    const { container } = render(() => (
      <ClaudeCodeSetup {...baseProps} apiKey="htrn_full_secret_value" keyPrefix="htrn_full" />
    ));
    // Masked by default.
    expect(container.textContent).toContain("htrn_full...");
    expect(container.textContent).not.toContain("htrn_full_secret_value");

    const reveal = container.querySelector('[aria-label="Reveal API key"]') as HTMLButtonElement;
    expect(reveal).not.toBeNull();
    fireEvent.click(reveal);
    expect(container.textContent).toContain("htrn_full_secret_value");
  ...
  it("copy button always uses the real key (not the masked one)", () => {
    ...
    expect(writeText.mock.calls[0][0]).toContain("htrn_full_secret_value");
