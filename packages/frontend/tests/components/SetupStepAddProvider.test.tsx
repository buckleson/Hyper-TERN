...
describe("SetupStepAddProvider", () => {
  ...
  it("renders heading", () => {
    ...
    expect(screen.getByText("Connect your agent to Hyper-Tern")).toBeDefined();
  });
  ...
  it("shows Craft setup when Craft Agent tab clicked", () => {
    ...
    expect(container.textContent).toContain("Hyper-Tern provider preset");
    expect(container.textContent).toContain("htrn_YOUR_KEY");
  });
  ...
  it("shows OpenCode setup when OpenCode tab clicked", () => {
    ...
    expect(container.textContent).toContain('"model": "hyper-tern/auto"');
  });
  ...
  it("shows Agents card when OpenClaw selected", () => {
    ...
    expect(container.textContent).toContain("hyper-tern/auto");
  });
  ...
  it("shows Hermes setup when Hermes Agent selected", () => {
    ...
    expect(container.textContent).toContain("Point Hermes at the Hyper-Tern endpoint");
  });
  ...
  it("switches back to OpenClaw from Hermes Agent", () => {
    ...
    expect(container.textContent).toContain("hyper-tern/auto");
    expect(container.textContent).not.toContain("Point Hermes at the Hyper-Tern endpoint");
  });
  ...
  it("shows onboard fields on wizard sub-tab", () => {
    ...
    expect(fields[4].textContent).toContain("hyper-tern");
  });

  it("shows eye toggle on CLI sub-tab when apiKey provided", () => {
    const { container } = render(() => (
      <SetupStepAddProvider {...defaultProps} apiKey="htrn_secret" keyPrefix="htrn_abc" />
    ));
  ...
  it("reveals key in CLI commands when eye toggle clicked", () => {
    ...
    expect(container.textContent).toContain("htrn_abc...");
    expect(container.textContent).not.toContain("htrn_secret");
    fireEvent.click(container.querySelector('[aria-label="Reveal API key"]')!);
    expect(container.textContent).toContain("htrn_secret");
  });
  ...
  it("does not show eye toggle on CLI when no apiKey", () => {
    const { container } = render(() => (
      <SetupStepAddProvider {...defaultProps} keyPrefix="htrn_abc" />
    ));
  ...
  it("shows full API key on Toolkits tab when provided", () => {
    const { container } = render(() => (
      <SetupStepAddProvider {...defaultProps} apiKey="htrn_test_key" />
    ));
    fireEvent.click(container.querySelectorAll(".setup-segment__btn")[1]);
    expect(container.textContent).toContain("htrn_test_key");
  });
  ...
  it("includes api key in CLI snippet", () => {
    const { container } = render(() => (
      <SetupStepAddProvider {...defaultProps} apiKey="htrn_test" keyPrefix="htrn_tes" />
    ));
    fireEvent.click(container.querySelector('[aria-label="Reveal API key"]')!);
    expect(container.textContent).toContain("htrn_test");
  });
  ...
  describe("platform-filtered mode", () => {
    it("shows OpenClawSetup directly when platform is openclaw", () => {
      ...
      expect(container.textContent).toContain("Register Hyper-Tern in your OpenClaw config");
    ...
    it("shows correct heading for openclaw", () => {
      ...
      expect(screen.getByText("Connect your OpenClaw agent to Hyper-Tern")).toBeDefined();
    });
    ...
    it("shows correct heading for hermes", () => {
      ...
      expect(screen.getByText("Connect your Hermes agent to Hyper-Tern")).toBeDefined();
    });
    ...
    it("shows correct heading for nanobot", () => {
      ...
      expect(screen.getByText("Connect your Nanobot agent to Hyper-Tern")).toBeDefined();
    });
    ...
    it("shows correct heading for craft", () => {
      ...
      expect(screen.getByText("Connect your Craft agent to Hyper-Tern")).toBeDefined();
    });
    ...
    it("shows correct heading for claude-code", () => {
      ...
      expect(screen.getByText("Connect Claude Code to Hyper-Tern")).toBeDefined();
    });
    ...
    it("shows correct heading for opencode", () => {
      ...
      expect(screen.getByText("Connect OpenCode to Hyper-Tern")).toBeDefined();
    });
    ...
    it("still shows heading in platform-filtered mode", () => {
      ...
      expect(container.textContent).toContain("Connect your OpenClaw agent to Hyper-Tern");
    });
    ...
    it("does not show description text in filtered mode", () => {
      ...
      expect(container.textContent).not.toContain("Point your agent at the Hyper-Tern endpoint");
    });
  });
});
