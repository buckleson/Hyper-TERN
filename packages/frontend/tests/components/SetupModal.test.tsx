...
vi.mock("../../src/services/api.js", () => ({
  getAgentKey: vi.fn().mockResolvedValue({ keyPrefix: "htrn_abc" }),
}));
...
describe("SetupModal", () => {
  ...
  it("shows description text", () => {
    ...
    expect(container.textContent).toContain("Connect your agent to Hyper-Tern");
  });
  ...
  it("passes apiKey prop to Add Provider step", () => {
    const { container } = render(() => (
      <SetupModal open={true} agentName="test-agent" apiKey="htrn_full_key" onClose={onClose} />
    ));
    const step = container.querySelector('[data-testid="step-add-provider"]');
    expect(step).not.toBeNull();
    expect(step!.getAttribute("data-key")).toBe("htrn_full_key");
  });

  it("computes production baseUrl on app.hyper-tern.build hostname", () => {
    const origLocation = window.location;
    Object.defineProperty(window, "location", {
      value: { ...origLocation, hostname: "app.hyper-tern.build", origin: "https://app.hyper-tern.build" },
      writable: true,
      configurable: true,
    });
    const { container } = render(() => (
    ...
    const step = container.querySelector('[data-testid="step-add-provider"]');
    expect(step?.getAttribute("data-base-url")).toBe("https://app.hyper-tern.build/v1");
  ...
  it("still shows setup content when apiKey prop is provided even if fetch returns null", async () => {
    ...
    const { container } = render(() => (
      <SetupModal open={true} agentName="fail-agent" apiKey="htrn_provided" onClose={onClose} />
    ));
    await vi.waitFor(() => {
      const step = container.querySelector('[data-testid="step-add-provider"]');
      expect(step).not.toBeNull();
      expect(step!.getAttribute("data-key")).toBe("htrn_provided");
    });
  });
  ...
  it("falls back to apiKeyData apiKey when props.apiKey is not provided", async () => {
    ...
    await vi.waitFor(() => {
      const step = container.querySelector('[data-testid="step-add-provider"]');
      expect(step).not.toBeNull();
      expect(step!.getAttribute("data-key")).toBe("htrn_fetched_key");
    });
  });
});
