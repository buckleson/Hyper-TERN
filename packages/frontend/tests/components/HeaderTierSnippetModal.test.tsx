...
describe("HeaderTierSnippetModal", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetAgentKey.mockResolvedValue({ apiKey: "htrn_abc", keyPrefix: "htrn_abc" });
  ...
  it("passes the tier's header_key/header_value as customHeaders to FrameworkSnippets", () => {
    ...
    const headers = JSON.parse(getByTestId("snippets-custom-headers").textContent ?? "{}") as Record<
      ...
      string
    >;
    expect(headers).toEqual({ "x-hyper-tern-tier": "premium" });
  });

  it("uses window.location.origin + /v1 as the base URL on non-app.hyper-tern.build hosts", () => {
    const { getByTestId } = render(() => (
      <HeaderTierSnippetModal agentName="demo" tier={baseTier} onClose={vi.fn()} />
    ));
  ...
  it("uses the production URL on app.hyper-tern.build", () => {
    Object.defineProperty(window, "location", {
      configurable: true,
      value: { hostname: "app.hyper-tern.build", origin: "https://app.hyper-tern.build" },
    });
    const { getByTestId } = render(() => (
      <HeaderTierSnippetModal agentName="demo" tier={baseTier} onClose={vi.fn()} />
    ));
    expect(getByTestId("snippets-base-url").textContent).toBe("https://app.hyper-tern.build/v1");
  });
  ...
  it("threads apiKey and keyPrefix from getAgentKey through to FrameworkSnippets", async () => {
    ...
    expect((await findByTestId("snippets-api-key")).textContent).toBe("htrn_abc");
    expect((await findByTestId("snippets-key-prefix")).textContent).toBe("htrn_abc");
  });
