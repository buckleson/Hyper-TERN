...
vi.mock("hyper-tern-shared", async (importOriginal) => {
  const actual = await importOriginal<typeof import("hyper-tern-shared")>();
...
describe("RoutingInstructionModal", () => {
  beforeEach(() => {
    mockGetAgentKey.mockResolvedValue({ keyPrefix: "htrn_abc", apiKey: "htrn_abc123" });
  });
  ...
  it("shows terminal with placeholder before model is selected in disable mode", () => {
    ...
    expect(container.textContent).toContain("openclaw config unset models.providers.hyper-tern");
    expect(container.textContent).toContain("openclaw config unset agents.defaults.models.hyper-tern/auto");
  });
  ...
  it("copies the disable command via copy button", async () => {
    ...
    expect(copiedText).toContain("openclaw config unset models.providers.hyper-tern");
    expect(copiedText).toContain("openclaw config unset agents.defaults.models.hyper-tern/auto");
  ...
  it("passes apiKey to SetupStepAddProvider in enable mode", async () => {
    ...
    await vi.waitFor(() => {
      const el = container.querySelector('[data-testid="setup-add-provider"]');
      expect(el).not.toBeNull();
      expect(el!.getAttribute("data-api-key")).toBe("htrn_abc123");
    });
  });
  ...
  it("passes null apiKey to SetupStepAddProvider when full key unavailable", async () => {
    mockGetAgentKey.mockResolvedValue({ keyPrefix: "htrn_abc", apiKey: null });
    const { container } = render(() => (
      <RoutingInstructionModal open={true} mode="enable" agentName="test-agent" onClose={() => {}} />
    ));
    await vi.waitFor(() => {
  ...
  it("passes full apiKey to SetupStepAddProvider when available", async () => {
    mockGetAgentKey.mockResolvedValue({ keyPrefix: "htrn_abc", apiKey: "htrn_abc123full" });
    const { container } = render(() => (
      <RoutingInstructionModal open={true} mode="enable" agentName="test-agent" onClose={() => {}} />
    ));
    await vi.waitFor(() => {
      const el = container.querySelector('[data-testid="setup-add-provider"]');
      expect(el).not.toBeNull();
      expect(el!.getAttribute("data-api-key")).toBe("htrn_abc123full");
    });
  });
