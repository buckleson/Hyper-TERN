...
describe("CraftAgentSetup", () => {
  ...
  it("renders the Hyper-Tern preset instruction", () => {
    const { container } = render(() => <CraftAgentSetup {...baseProps} />);
    expect(container.textContent).toContain("Hyper-Tern");
  ...
  it("masks the API key when only a prefix is available", () => {
    const { container } = render(() => (
      <CraftAgentSetup {...baseProps} keyPrefix="htrn_abc" />
    ));
  ...
  it("toggles the API key reveal when the eye button is clicked", () => {
    const { container } = render(() => (
      <CraftAgentSetup {...baseProps} apiKey="htrn_full_secret_value" keyPrefix="htrn_full" />
    ));
