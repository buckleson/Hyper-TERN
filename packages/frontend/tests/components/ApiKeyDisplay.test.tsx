...
describe("ApiKeyDisplay", () => {
  ...
  it("does not show warning message when apiKey is provided", () => {
    const { container } = render(() => (
      <ApiKeyDisplay apiKey="htrn_full_key_123" keyPrefix={null} />
    ));
  ...
  it("shows prefix hint when only keyPrefix is provided", () => {
    const { container } = render(() => (
      <ApiKeyDisplay apiKey={null} keyPrefix="htrn_abc" />
    ));
  ...
  it("does not show prefix hint when full key is provided", () => {
    const { container } = render(() => (
      <ApiKeyDisplay apiKey="htrn_full_key" keyPrefix="htrn_abc" />
    ));
  ...
  it("does not render api-key-display__warning class when apiKey is provided", () => {
    const { container } = render(() => (
      <ApiKeyDisplay apiKey="htrn_key" keyPrefix={null} />
    ));
  ...
  it("renders api-key-display__prefix class when only keyPrefix is provided", () => {
    const { container } = render(() => (
      <ApiKeyDisplay apiKey={null} keyPrefix="htrn_pre" />
    ));
  ...
  it("renders prefix inside a code element", () => {
    ...
    expect(code!.textContent).toBe("htrn_pre...");
  });
});
