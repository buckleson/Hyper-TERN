...
describe("Settings", () => {
  beforeEach(() => {
    ...
    mockGetAgentKey.mockResolvedValue({ keyPrefix: "htrn_abc" });
  ...
  it("shows reveal button when API returns full key", async () => {
    mockGetAgentKey.mockResolvedValue({ keyPrefix: "htrn_abc", apiKey: "htrn_full_key_123" });
    const { container } = render(() => <Settings />);
    await vi.waitFor(() => {
  ...
  it("hides full key when hide button is clicked", async () => {
    ...
    await vi.waitFor(() => {
      expect(container.textContent).not.toContain("htrn_full_key_123");
    });
  });
  ...
  it("clicking Rotate key calls rotateAgentKey", async () => {
    mockRotateAgentKey.mockResolvedValue({ apiKey: "htrn_new_rotated_key" });
    const { container } = render(() => <Settings />);
  ...
  it("auto-reveals key after successful rotation", async () => {
    ...
    await vi.waitFor(() => {
      expect(container.querySelector('[aria-label="Hide API key"]')).not.toBeNull();
      expect(container.textContent).toContain("htrn_new_rotated_key");
    });
  });
  ...
  it("passes keyPrefix to setup component", async () => {
    mockGetAgentKey.mockResolvedValue({ keyPrefix: "htrn_xyz_prefix" });
    const { container } = render(() => <Settings />);
    await vi.waitFor(() => {
      const el = container.querySelector('[data-testid="setup-add-provider"]');
      expect(el).not.toBeNull();
      expect(el!.getAttribute("data-key-prefix")).toBe("htrn_xyz_prefix");
    });
  });
  ...
  it("shows fallback key display when no full key available", async () => {
    mockGetAgentKey.mockResolvedValue({ keyPrefix: "htrn_xyz" });
    const { container } = render(() => <Settings />);
    await vi.waitFor(() => {
      expect(container.textContent).toContain("htrn_xyz...");
    });
  });
  ...
  it("uses app.hyper-tern.build URL when hostname matches", async () => {
    const originalLocation = window.location;
    Object.defineProperty(window, "location", {
      value: { ...originalLocation, hostname: "app.hyper-tern.build", origin: "https://app.hyper-tern.build" },
      writable: true,
      configurable: true,
    });
    mockGetAgentKey.mockResolvedValue({ keyPrefix: "htrn_abc" });
    ...
    await vi.waitFor(() => {
      const el = container.querySelector('[data-testid="setup-add-provider"]');
      expect(el).not.toBeNull();
      expect(el!.getAttribute("data-base-url")).toBe("https://app.hyper-tern.build/v1");
    });
    Object.defineProperty(window, "location", { value: originalLocation, writable: true, configurable: true });
  });
