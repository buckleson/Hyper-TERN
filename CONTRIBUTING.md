...
The full NestJS + SolidJS stack runs locally against PostgreSQL, and the same database backend is used in the [cloud version](https://app.hyper-tern.build). For local development, the simplest option is to run PostgreSQL in Docker and point `DATABASE_URL` at it.
...
Hyper-Tern is a monorepo managed with [Turborepo](https://turbo.build/) and npm workspaces.
...
```bash
git clone https://github.com/<your-username>/hyper-tern.git
cd hyper-tern
npm install
```
...
```bash
docker run --name hyper-tern-postgres \
...
```bash
docker start hyper-tern-postgres   # start again later
docker stop hyper-tern-postgres    # stop without deleting data
docker rm -f hyper-tern-postgres   # remove the container
```
...
5. With `SEED_DATA=true`, you can log in with `admin@hyper-tern.build` / `hyper-tern`.

## Testing Routing with AI agents

Hyper-Tern is a smart router for any AI agent that speaks OpenAI-compatible HTTP. The list of supported agents lives in `packages/shared/src/agent-type.ts` — OpenClaw, Hermes, OpenAI SDK, Vercel AI SDK, LangChain, and cURL are all first-class. The dashboard's "Connect Agent" flow generates the right setup snippet for whichever platform you pick.
...
To test routing against your local backend, add Hyper-Tern as a model provider in your OpenClaw config:

1. Build and start the backend in self-hosted mode:

```bash
npm run build
HYPER_TERN_MODE=selfhosted PORT=38238 BIND_ADDRESS=127.0.0.1 \
...
3. Add Hyper-Tern as a provider in OpenClaw:
...
Hyper-Tern ships as the Docker image at [`hypertern/hyper-tern`](https://hub.docker.com/r/hypertern/hyper-tern). Releases are manual:

1. After merging PRs with changesets, a `chore: version packages` PR will be open on `main` — merge it to land the version bump in `packages/hyper-tern/package.json` and update `packages/hyper-tern/CHANGELOG.md`.
2. Go to **GitHub Actions → Docker → Run workflow**, leave the `version` input blank, click Run.
3. The workflow reads the version from `packages/hyper-tern/package.json` and pushes `hypertern/hyper-tern:{version}` (plus `{major}.{minor}`, `{major}`, and a `sha-<short>` rollback tag).
