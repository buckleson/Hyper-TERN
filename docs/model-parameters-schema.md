...
catalog. The catalog is metadata: it describes which request parameters Hyper-Tern
...
variants use the `-subscription` model suffix. Hyper-Tern still refreshes
...
- `default` is the provider default Hyper-Tern should display when known.
...
2. merges configured Hyper-Tern values last, so Hyper-Tern values win for the same provider path
3. omits params that are not applicable under the final effective values

Client request body values that do not overlap configured Hyper-Tern model
